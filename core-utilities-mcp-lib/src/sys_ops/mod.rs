...
pub fn get_system_context() -> Result<Value, String> {
...
pub async fn execute_command_in_sandbox(command: &str) -> Result<Value, String> {
    ...
    let mut stdout = child.stdout.take().ok_or("Failed to open stdout pipe")?;
    let mut stderr = child.stderr.take().ok_or("Failed to open stderr pipe")?;
