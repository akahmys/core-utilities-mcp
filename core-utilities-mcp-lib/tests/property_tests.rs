...
use core_utilities_mcp_lib::guardrails::{truncate_output, validate_path_safety, ENV_MUTEX};
...
fn default_limit() -> (usize, tokio::sync::MutexGuard<'static, ()>) {
    let lock = ENV_MUTEX.blocking_lock();
...
proptest! {
    ...
    fn content_is_always_a_prefix_of_the_input(input in "[a-zA-Z0-9 \n]{0,20000}") {
        let (_limit, _lock) = default_limit();
        let result = truncate_output(&input);
