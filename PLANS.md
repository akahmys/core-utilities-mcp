...
2. The user is reconsidering whether the remaining 14 tools all belong under "core utilities," with particular scrutiny on `execute_command_in_sandbox` (a fully generic, unstructured shell escape hatch that arguably undermines the "deterministic, structured, safe" pitch of every other tool). No decision yet — awaiting the user's input.
